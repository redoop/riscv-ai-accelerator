...
系统提供三种不同的波形查看模式，适用于不同场景：

### 1. 流式加载模式（默认，推荐）

**访问地址**: http://localhost:5000 或 http://localhost:5000/

**特点**:
- ✅ 支持任意大小的 VCD 文件（包括 GB 级别）
- ✅ 按需加载，只渲染可见区域
- ✅ 内存占用极低
- ✅ 滚动时自动加载新区域
- ✅ 支持大量信号同时显示
...
**访问地址**: http://localhost:5000/image
...
**访问地址**: http://localhost:5000/canvas

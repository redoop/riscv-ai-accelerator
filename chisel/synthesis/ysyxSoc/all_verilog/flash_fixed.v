...
module flash (
  ...
  parameter cmd_t=0, addr_t=1, data_t=2, err_t=3;
  reg [2:0] state_t;
  reg [2:0]  state;
  reg [7:0]  counter;
  reg [7:0]  cmd;
  ...
  always@(posedge sck or posedge reset) begin
    if (reset) state <= cmd_t;
      ...
      case (state)
        ...
        default: begin
          state <= err_t;  // Go to error state for unsupported commands
          // // $fwrite(32'h80000002, "Assertion failed: Unsupported command `%xh`, only support `03h` read command\n", cmd);
          // $fatal; // Not synthesizable - removed for synthesis
        end
      endcase
    end
  end
...
// Synthesizable flash_cmd module - replaces DPI-C version
// For synthesis, we use a simple ROM that returns zeros
// In real hardware, this would connect to actual flash memory
module flash_cmd(
  input             clock,
  input             valid,
  input       [7:0] cmd,
  ...
  always@(posedge clock) begin
    if (valid) begin
      if (cmd == 8'h03) begin
        // Simple ROM - returns zero for synthesis
        // In real implementation, this would read from flash
        data <= 32'h0;
      end else begin
        data <= 32'hDEADBEEF; // Error indicator
      end
    end
  end
endmodule

...
选项:
    -f, --file FILE         VCD 文件路径 (默认: waves/post_syn.vcd)
...
示例:
    $0                                      # 使用默认参数
    $0 -f waves/my.vcd                      # 指定 VCD 文件
    $0 -s 20 -p 3000                        # 更多信号和采样点
    $0 -f waves/my.vcd -o my_wave.html      # 指定输出文件
...
# 构建命令
CMD="python3 generate_static_wave.py \"$VCD_FILE\" --max-signals $MAX_SIGNALS --max-points $MAX_POINTS"

...
**v0.4.1 优化（2025-12-03）：**
- **IO Pad 优化**: 从 97 个减少到 61 个，满足 81-pad 限制
- **GPIO 调整**: 32-bit → 16-bit (足够大多数应用场景)
- **裕量提升**: 从 -16 提升到 +20 (24.7% 裕量)
- **核心功能**: 完整保留所有关键特性 (Quad SPI, Flash, PSRAM, LCD, UART)
- **测试覆盖**: 100% 通过 (57/57 测试)

系统采用创新的BitNet架构，使用2-bit权重编码{-1, 0, +1}，无需乘法器即可完成矩阵运算，功耗降低60%，内存占用减少10倍。系统峰值性能达到6.4 GOPS @ 100MHz，目标功耗小于100mW。所有模块均已通过ChiselTest验证，测试覆盖率达到100% (57/57测试通过)。
...
**端口资源统计（v0.4.1）：**
- 输入端口：23个（clock + reset + UART RX + GPIO IN[15:0] + Flash MISO + PSRAM MISO + PSRAM SIO2/3 IN）
- 输出端口：38个（UART TX + LCD SPI + GPIO OUT[15:0] + Flash SPI + PSRAM SPI/Quad SPI + 中断信号）
- 总端口数：61个（相比 v0.4 减少 36 个端口，满足 81-pad 限制）
- 裕量：+20 个端口（24.7% 裕量）

本设计的SoC顶层端口功能如表1所示。

**表1 SoC设计顶层端口功能说明（v0.4.1）**
...
| **GPIO通用IO接口** | | | **v0.4.1 优化** |
| io_gpio_out[15:0] | output | 16 | GPIO输出数据，16位通用数字输出（v0.4.1优化：32-bit→16-bit） |
| io_gpio_in[15:0] | input | 16 | GPIO输入数据，16位通用数字输入（v0.4.1优化：32-bit→16-bit） |
...
| **端口统计（v0.4.1）** | | | |
| 输入端口总数 | - | 23 | clock(1) + reset(1) + UART(1) + GPIO(16) + Flash(1) + PSRAM(3) |
| 输出端口总数 | - | 38 | UART(3) + LCD(6) + GPIO(16) + Flash(3) + PSRAM(9) + 中断(3) |
| 总端口数 | - | 61 | v0.4.1 实际使用61个端口（相比v0.4减少36个，满足81-pad限制） |

**说明：** 
- 本设计为纯数字逻辑设计，所有端口均为数字信号
- GPIO端口可根据实际需求配置为输入或输出
- Flash和PSRAM接口支持标准SPI协议，PSRAM额外支持Quad SPI模式
- v0.4.1优化：GPIO从32-bit减少到16-bit，满足81-pad限制，保留24.7%裕量
- 16个GPIO足够支持大多数应用场景（LED、按钮、传感器、控制信号等）
...
### v0.4.1 (2025-12-03) - IO Pad 优化

**主要变更：**
- IO Pad 优化：从 97 个减少到 61 个（减少 37.1%）
- GPIO 调整：32-bit → 16-bit
- 裕量提升：从 -16 提升到 +20（24.7% 裕量）
- 满足 81-pad 限制要求
...
- 输入端口：39 → 23（减少 16 个）
- 输出端口：58 → 38（减少 20 个）
- GPIO：io_gpio_out[31:0] → io_gpio_out[15:0]
- GPIO：io_gpio_in[31:0] → io_gpio_in[15:0]

**测试结果：**
- 测试通过率：100% (57/57)
- GPIO 测试：✅ 通过（16-bit 读写验证）
- 核心功能：完整保留（Quad SPI, Flash, PSRAM, LCD, UART）

**实施时间：** 45 分钟  
**代码变更：** ~20 行（3 个文件）  
**风险等级：** 极低

### v0.4 (2025-12-03) - Flash + PSRAM 扩展

**主要变更：**
- 新增 16 MB SPI Flash 控制器
- 新增 8 MB PSRAM 控制器（支持 Quad SPI）
- 存储容量提升 375 倍（64 KB → 24 MB）
- Quad SPI 性能提升 4 倍（6.25 MB/s → 25 MB/s）
...
### v0.2 (2025-11-16) - 外设扩展

**主要变更：**
- RealUART 控制器（115200 bps, FIFO, 中断）
- TFTLCD SPI 控制器（ST7735, 128x128 RGB565）
- Bootloader 系统
- 图形库和软件栈

### v0.1 (2025-11-14) - 基础版本

**主要变更：**
- PicoRV32 处理器核心
- CompactAccel 加速器（8x8 矩阵）
- BitNetAccel 加速器（8x8 无乘法器）
- 基础 GPIO 和内存接口

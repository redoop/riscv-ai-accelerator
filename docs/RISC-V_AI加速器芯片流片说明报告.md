...
（一）添加IOPad的asic_top的顶层RTL仿真
  ![图3.3 asic_top的顶层RTL仿真波形图](image/asic_top_rtl.jpg)



（二）V1210的DC网表仿真（用于构建带IOPad的网表测试环境）
  ![图3.4 V1210的DC网表仿真波形图](image/asic_top_rtl1.jpg)


（三）云平台版本的单独设计网表仿真
  ![图3.5 云平台版本的单独设计网表仿真](image/asic_top_rtl2.jpg)
...
本设计计划使用VCS进行原型验证,布局布线后将进行完整的系统级测试，包括：
